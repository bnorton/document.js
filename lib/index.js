...
Document = Object.progeny('Document', {
  ...
  get: function(name) {
    ...
    if(this.class.belongsTo.indexOf(name) >= 0) {
      ...
      return (name in this._data) ? this._data[name] : null;
  ...
  set: function() { var name, from, to, options = arguments[0],
    ...
    for(name in options) {
      ...
      if(belongsTo || this.class.namedFields[name]) {
        to = typeof options[name] === 'undefined' ? null : options[name];

        if((from = this.get(name)) != to) {
  ...
  asJSON: function() {
    ...
    for(var i = 0; i < allow.length; ++i) {
      ...
      if(/(.+)_id$/.test(name)) {
        ...
        json[shortName] || (json[shortName] = { id: value });
      } else {
        value = this.get(name);
        value = typeof (value && value.asJSON) === 'function' ? value.asJSON() : value;
      }
